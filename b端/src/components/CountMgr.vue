  ...
  <div class="container">
    <div class="row form-group">
      <div class="col col-lg-4">
        <select v-model="year" @click="changeYear()" class="form-control" style="width:40%">
          <option disabled value="">请选择年份</option>
          <option :value="y" v-for="(y, index) in years" :key="index">{{ y }}</option>
        </select>
      </div>
      <div class="col col-lg-4">
        <select v-model="way" @change="selectWay()" class="form-control" style="width:40%">
      ...
      <div class="col col-lg-4">
        <select v-model="month" @change="selectMonth()" class="form-control" style="width:40%">
  ...
  export default {
    ...
    methods: {
      ...
      getMonthKind() {
        ...
        this.axios
          .get(
            `${this.host}/countKindVolumeByMonth?year=${this.year
            }&month=${this.month}`
          )
          .then(kind => {
      ...
      getMonthVolumeTen() {
        this.axios
          .get(
            `${this.host}/selectTopVolumeByMonth?year=${this.year
            }&month=${this.month}`
          )
          .then(books => {
