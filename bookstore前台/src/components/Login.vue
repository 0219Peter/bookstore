...
export default {
    ...
    methods:{
        login: function(){
                ...
                //         alert("登录失败,用户名或密码错误")
                //         }) ;
                if(this.username === this.register.name)
                 {
                    if(this.userpassword === this.register.password)
                    {
                        this.$store.commit('SET_USER', {username:this.username,password:this.userpassword})
                        this.$store.commit('SET_ONLINE', true);
                        this.online = this.$store.getters.online;
                        alert("登录成功")
                    }
