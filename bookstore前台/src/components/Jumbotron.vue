...
<script>
  import{fetchJumbotron,fetchDeleteJumbotron} from '../api.js'
...
export default {
   ...
   async mounted(){
        const data = await fetchJumbotron()
        this.caritem =data.orders
    },
    computed:{
      total: function(){
            let totals=0;
